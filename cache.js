(function e(t, n, r) {
    ...
    1: [function(require, module, exports) {
        (function($, window, document, undefined) {
            ...
            $.fn[NAME] = function(options) {
                ...
                var opts = $.extend({}, $.fn[NAME].defaults, options), delimiterRegex, Element = {};
                ...
                this.each(function() {
                    ...
                    if (options !== false) {
                        ...
                        if ($this.data(NAME) !== undefined && ($this.data(NAME) !== "search" || opts.search === false)) {
                            reverse($this, opts);
                            if (opts.debug)
                                console.log(NAME + ": Removed element's existing Blast call.")
                        }
                        ...
                        try {
                            ...
                            opts.tag = "span";
                            if (opts.debug)
                                console.log(NAME + ": Invalid tag supplied. Defaulting to span.")
                        }
    ...
    8: [function(require, module, exports) {
        var breakpoint = require("./_breakpoint");
    ...
    9: [function(require, module, exports) {
        module.exports = function() {
            var $entCalc = $(".js-entCalc");
            $entCalc.each(function(index, el) {
                var $el = $(el);
                var $entTierPrice = $($el.find(".js-entTierPrice"));
                var $entInput = $el.find(".js-entInput");
                var $entEdgeNodes = $el.find(".js-entEdgeNodes");
                var $entAccess = $el.find(".js-entAccess");
                var $entUsers = $el.find(".js-entUsers");
                var $entRadios = $el.find(".js-entRadios");
                var options = {};
                var setInputs = function() {
                    $entInput.each(function() {
                        var self = this;
                        var feat = self.getAttribute("data-ent").toString();
                        var featPrice = parseInt(self.getAttribute("data-price").toString(), 10);
                        options[feat] = options[feat] || {};
                        if (self.checked) {
                            options[feat].price = featPrice;
                            options[feat].check = self.checked;
                            if (feat === "nodes") {
                                options[feat].count = parseInt(self.value, 10)
                            }
                            if (feat === "branding" && featPrice === 0) {
                                options["nodes"].count = 0;
                                resetRadios($entEdgeNodes);
                            }
                            if (feat === "access") {
                                if (featPrice === 0) {
                                    options[feat].count = 0
                                } else {
                                    options[feat].count = parseInt($(".js-entUsers").get(0).value, 10)
                                }
                            }
                        }
                    })
                };
                $entUsers.each(function() {
                    var $self = $(this);
                    var userCost = function() {
                        resetRadios($entAccess, 1);
                        setInputs();
                        if (options.access.count >= 2e3) {
                            $entTierPrice.parent().parent().find(".js-callUs").removeClass("is-hidden");
                            $entTierPrice.parent().addClass("is-hidden")
                        } else {
                            $entTierPrice.parent().parent().find(".js-callUs").addClass("is-hidden");
                            $entTierPrice.parent().removeClass("is-hidden");
                            $entTierPrice.text(calcPrice(options))
                        }
                    };
                ...
                var resetRadios = function($el, index) {
                    index = index || 0;
                    $el.each(function() {
                        var $self = $(this);
                        var $entRadio = $self.find(".js-entInput");
                        $entRadio.each(function() {
                            var self = this;
                            self.removeAttribute("checked")
                        });
                        $entRadio.get(index).checked = true;
                        $entRadio.get(index).setAttribute("checked", "checked")
                    })
                };
                var resetOpenSource = function() {
                    $(".js-entNotOpenSource").each(function() {
                        var $self = $(this);
                        var $entRadio = $self.find(".js-entInput");
                        $entRadio.each(function() {
                            var self = this;
                        ...
                        $entRadio.get(0).checked = true;
                        $entRadio.get(0).setAttribute("checked", "checked")
                    })
                };
                $entEdgeNodes.each(function() {
                    var $self = $(this);
                    var $entRange = $self.find(".js-entRange");
                    var $entRangeOutput = $self.find(".js-entRangeOutput");
                    var $entRadio = $self.find(".js-entInput");
                    $entRange.on("input", function() {
                        var self = this;
                        var feat = self.getAttribute("data-ent").toString();
                        options[feat] = options[feat] || {};
                        options[feat].count = parseInt(self.value, 10);
                        $entRadio.get(1).value = parseInt(self.value, 10);
                        resetOpenSource();
                        $entRangeOutput.get(0).value = self.value >= 12 ? "âˆž" : parseInt(self.value, 10);
                        if (self.value > 0) {
                            $entRadio.get(1).checked = true;
                            $entRadio.each(function() {
                                var self = this;
                                self.removeAttribute("checked")
                            })
                ...
                $entRadios.change(function() {
                    var self = this;
                    var feat = self.getAttribute("data-ent");
                    var featPrice = parseInt(self.getAttribute("data-price"), 10);
                    setInputs();
                    $entTierPrice.text(calcPrice(options))
                });
                setInputs();
        ...
        ;
        var calcPrice = function(opts) {
            var $entOutput = $(".js-entOutput");
            var users = opts.access.count || 0;
            var price = (opts.nodes.count - 1) * opts.nodes.price;
            price = price + (users - 1) * opts.access.price;
            for (var item in opts) {
                if (opts.hasOwnProperty(item)) {
                    if (opts[item].check) {
                        price = price + opts[item].price;
                        var output = $entOutput.find('*[data-ent="' + item + '"]');
                        output.text(opts[item].price);
                        if (opts[item].count) {
                            output.parent().find(".js-entOutputPreface").text(opts[item].count + " Ã— ")
                        }
                    }
                }
            }
    ...
    10: [function(require, module, exports) {
        ...
        require("./vendor/jquery-ajaxchimp.js");
        require("./vendor/jquery-details.js");
        var randomBetween = require("./_random-between.js");
        var smoothscroll = require("smoothscroll");
        var ent = require("./enterprise.js")();
        var toc = require("./_toc.js")();
        var $featureExampleCli = $(".js-cli--demo");
        var $featureExampleShake = $(".js-featureShake");
        var $featureExampleDiagram = $(".js-featureDiagram");
        var $featureExampleRotate = $(".js-featureRotate");
        var $ajaxForm = $(".js-mcEmbeddedSubscribeForm");
        $("html").removeClass("no-js");
        $("details").details();
        var formSetup = $ajaxForm.ajaxChimp({
            url: "https://surge.us11.list-manage.com/subscribe/post-json?u=e25abb0e26e5a5ecb16867791&amp;id=6f960155bb",
            errorSelector: ".js-mceErrorResponse",
            successSelector: ".js-mceSuccessResponse",
        ...
        $(".js-line").each(function() {
            var $self = $(this);
            $self.blast({
                delimiter: $self.attr("data-delimiter") || "character",
                tag: "span",
                customClass: "js-cli" + ($self.attr("data-delimiter") || "character")
            })
        });
        $featureExampleCli.waypoint(function() {
            var $pre = $(this.element);
            var $el = $pre.find(".blast");
            var $result = $($pre.attr("data-result")) || false;
            var runningTotal = 550;
            $pre.removeClass("is-invisible").velocity("transition.slideUpIn", {
                duration: 1e3
            });
            $el.each(function(index) {
                var val = randomBetween(125, 20);
                var $self = $(this);
                var $delimiter = $self.parent().attr("data-delimiter");
                var selfDelay = parseInt($self.parent().attr("data-delay"), 10) || 0;
                if ($delimiter === "element" || $delimiter === "sentence") {
                    val = 325
                }
                runningTotal = runningTotal + val;
                $self.delay(runningTotal).velocity({
                    opacity: [1, 0]
                }, {
                    duration: 100,
                    stagger: 50,
                    complete: function() {
                        if (index + 1 === $el.length && $result) {
                            $result.removeClass("is-invisible").velocity("transition.slideUpIn", {
                                duration: 750
                            })
                        }
                    }
                });
                runningTotal = runningTotal + randomBetween(selfDelay, selfDelay * .25)
            });
            this.destroy()
        }, {
            offset: "90%"
        });
        $featureExampleDiagram.waypoint(function() {
            var $el = $(this.element);
            $el.removeClass("is-invisible").velocity("transition.slideUpIn", {
                duration: 750
            });
            this.destroy()
        }, {
        ...
        $featureExampleRotate.on("deviceorientation", function() {
            alert("hello")
        });
        $featureExampleShake.waypoint(function() {
            var $el = $(this.element);
            $el.addClass("shake");
            $el.hover(function(e) {
        ...
        window.addEventListener("deviceorientation", function(e) {
            $featureExampleRotate.css("webkitTransform", "rotate(" + e.gamma * 1.8 + "deg)").css("transform", "rotate(" + e.gamma * 1.8 + "deg)")
        }, false)
    }
    , {
        "./_random-between.js": 7,
        "./_toc.js": 8,
        "./enterprise.js": 9,
        "./vendor/jquery-ajaxchimp.js": 11,
        "./vendor/jquery-details.js": 12,
        "blast-text": 1,
        smoothscroll: 2,
        "velocity-animate": 3,
        "velocity-animate/velocity.ui": 4,
        "waypoints/lib/jquery.waypoints.js": 5
    }],
        ...
        (function(global) {
            (function(factory) {
                ...
                $.ajaxChimp.getTranslation = function(str, language, string_key) {
                    if ($.ajaxChimp.all_translations[language] && $.ajaxChimp.all_translations[language][string_key]) {
                ...
                $.fn.ajaxChimp = function(options) {
                    $.ajaxChimp.all_translations = $.extend({}, $.ajaxChimp.defaultTranslations, $.ajaxChimp.translations);
                    var deferreds = $(this).map(function(i, elem) {
                        ...
                        form.on("submit", function(e) {
                            ...
                            $.ajax({
                                ...
                                if (data.result === "success") {
                                    ...
                                    if (success_div.length !== 0) {
                                        msg = $.ajaxChimp.getTranslation($.ajaxChimp.successMessage + request_data.EMAIL, settings.language, "success");
                                        error_div.text("").hide();
                            ...
                            if (success_div.length !== 0) {
                                var submitMsg = $.ajaxChimp.getTranslation($.ajaxChimp.submitMessage, settings.language, "submit");
                                error_div.text("").hide();
